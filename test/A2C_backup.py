...
class Config:
    ...
    hidden_size: int = 256          # FIXED: increased from 128 (more capacity)
    grad_clip: float = 0.5          # gradient clipping threshold

    save_dir: str = "checkpoints"
    save_name: str = f"a2c_{rollout_steps}_{max_updates}.pt"
...
def collect_rollout(env: gym.Env, policy: PolicyNet, value: ValueNet,
                   ...
                   current_obs: np.ndarray, current_done: bool) -> Tuple[Dict, np.ndarray, bool, List[float]]:
    """
    ...
    for step in range(rollout_steps):
        # Select action
        obs_t = torch.tensor(current_obs, dtype=torch.float32, device=device).unsqueeze(0)
    ...
    return rollout_data, current_obs, current_done, episode_returns
...
@torch.no_grad()
def evaluate(cfg: Config, policy: PolicyNet, device: torch.device = torch.device('cpu'), update_idx: int = 0) -> float:
...
def train(cfg: Config):
    ...
    # Adam with recommended RL settings
    opt_policy = optim.Adam(policy.parameters(), lr=cfg.lr_policy, betas=(0.9, 0.999), eps=1e-5)
    opt_value = optim.Adam(value.parameters(), lr=cfg.lr_value, betas=(0.9, 0.999), eps=1e-5)
    ...
    for update_idx in range(1, cfg.max_updates + 1):
        # Collect rollout
        rollout_data, current_obs, current_done, episode_returns = collect_rollout(
        ...
        with torch.no_grad():
            ...
            if last_terminated == 1.0:
                ...
                next_obs_t = torch.tensor(current_obs, dtype=torch.float32, device=device).unsqueeze(0)
        ...
        if update_idx % cfg.eval_every == 0:
            policy.eval()
            avg_eval = evaluate(cfg, policy, device, update_idx)
            policy.train()
    ...
    plot_performance(history, save_path="training_performance_a2c.png")
