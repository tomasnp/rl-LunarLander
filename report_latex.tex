...
\end{itemize}
Dans la version finalisée du dépôt, trois runs longues (2000 épisodes) sont disponibles avec des dashboards PNG :
\begin{itemize}
    \item \file{dqn\_ep2000\_lr0.001\_epsd0.995\_buf10000\_tgt10\_hid128\_e2000\_1.png} (baseline) ;
    \item \file{dqn\_ep2000\_lr0.001\_epsd0.997\_buf10000\_tgt10\_hid128\_e2000\_2.png} ($\epsilon$-decay plus lent) ;
    \item \file{dqn\_ep2000\_lr0.001\_epsd0.995\_buf50000\_tgt10\_hid128\_e2000\_3.png} (replay buffer plus grand).
\end{itemize}

\paragraph{Courbes d'apprentissage DQN (dashboard)}
\begin{figure}[H]
    \centering
...
\begin{itemize}
    \item \textbf{DQN} : meilleur sweep (replay buffer 50k), $268.3 \pm 31.7$ sur 100 épisodes, \textbf{95\%} de succès (Tableau~\ref{tab:dqn_sweep}).
    \item \textbf{REINFORCE (+ baseline)} : hidden=256, $243.5 \pm 53.7$ sur 100 épisodes, \textbf{89\%} de succès (Tableau~\ref{tab:reinforce_sweep}).
    \item \textbf{A2C + GAE} : meilleur checkpoint sauvegardé (hidden=512) avec atteinte du critère ``Solved'' dans nos logs.
\end{itemize}
...
REINFORCE (+ baseline) & Oui (en test déterministe) & Faible à moyenne & Moyenne \\
